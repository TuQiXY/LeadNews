...
<h2>1.解决md5加密每次数据一样的问题</h2>
- 方法：加盐 手动加密，随机生成字符串
  用户密码＋字符串以md5加密方式存储在数据库中

<h2>2.报redis配置找不到，原因是找不到redis服务</h2>
<h4>解决方案一：在heima-leadnews-user服务中的bootstrap.yml文件中添加配置忽略，如下：</h4>
- spring:
  #忽略掉配置类
  autoconfigure:
...
<h4>解决方案二：在heima-leadnews-user服务中的bootstrap.yml文件中添加redis配置（需要安装redis服务）</h4>
- spring:
   redis:
   host: localhost
   port: 6379
