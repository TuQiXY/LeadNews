...
public class ApArticleServiceImpl  extends ServiceImpl<ApArticleMapper, ApArticle> implements ApArticleService {
    ...
    public ResponseResult getByChannelId(Integer channelId) {
        ...
        wrapper.eq(ApArticle::getChannelId,channelId);
        List<ApArticle> list = this.list(wrapper);
        ...
        if (StringUtils.isNotBlank(articleListStr)) {
            ...
            if (flag) {
                if (hotArticleVoList.size() >= 30) {
                    hotArticleVoList = hotArticleVoList.stream().sorted(Comparator.comparing(HotArticleVo::getScore).reversed()).collect(Collectors.toList());//按照分值倒叙
